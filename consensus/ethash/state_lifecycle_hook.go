// Ethernova: Phase 5 lifecycle hook for the ethash consensus engine.
//
// Mirrors consensus/lyra2/state_lifecycle_hook.go byte-for-byte except
// for the package declaration. Both engines must execute identical
// lifecycle bookkeeping; if the chain is mining via ethash, this is
// the path that runs.
//
// CONSENSUS-CRITICAL: this file MUST stay in lockstep with the lyra2
// version. Any divergence between the two = miners running different
// engines will produce different external lifecycle indexes, leading
// to divergent witness-restoration outcomes downstream. Treat both
// files as a single logical unit; update them together.
...
// runStateLifecycle is invoked from Finalize. It records the set of
// accounts touched during this block in the external Phase 15+5
// index, then runs a bounded sweep that flips any still-untouched
// accounts into the Archived tier and snapshots their storage root.
//
...
func runStateLifecycle(statedb *state.StateDB, currentBlock uint64) {
	...
	log.Debug("StateLifecycle (ethash): sweep complete",
		"block", currentBlock,
