...
func TestHeaderPhase10DResourceRLPWithNilEthereumOptionals(t *testing.T) {
	header := &Header{
		Difficulty: big.NewInt(1),
		Number:     big.NewInt(1),
		GasLimit:   30_000_000,
		Time:       1,
		ResourceUsed: &ResourceLimits{
			Compute: 7,
		},
		ResourceBasePrice: &ResourceLimits{
			Compute:     10_000,
			StateRead:   10_000,
			StateWrite:  10_000,
			ProtocolOps: 10_000,
	...
	if err := rlp.DecodeBytes(encoded, &decoded); err != nil {
		t.Fatalf("decode header with Phase 10D tail and nil Ethereum optionals: %v", err)
	}
	...
	if decoded.ResourceUsed == nil || !decoded.ResourceUsed.Equal(*header.ResourceUsed) {
		t.Fatalf("resourceUsed mismatch: got %#v want %#v", decoded.ResourceUsed, header.ResourceUsed)
	}
	if decoded.ResourceBasePrice == nil || !decoded.ResourceBasePrice.Equal(*header.ResourceBasePrice) {
		t.Fatalf("resourceBasePrice mismatch: got %#v want %#v", decoded.ResourceBasePrice, header.ResourceBasePrice)
	}
	...
	if !bytes.Equal(reencoded, encoded) {
		t.Fatalf("header RLP round-trip changed:\ngot  %x\nwant %x", reencoded, encoded)
	}
}
