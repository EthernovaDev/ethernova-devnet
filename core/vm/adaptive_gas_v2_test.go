...
func TestClassifyExecution_Light_SmallSSTORE(t *testing.T) {
	// 1-2 SSTORE with low score should be Light (token transfer pattern)
	tc := &TraceCounters{
	...
	t.Logf("SmallSSTORE: category=%s, score=%d", cat, score)
	if cat != ExecCategoryLight {
		t.Errorf("expected ExecCategoryLight for 2 SSTORE (token transfer), got %s", cat)
	}
}
...
func TestClassifyExecution_Deterministic_1000Runs(t *testing.T) {
	...
	firstAdj := ComputeGasAdjustment(firstCat, firstScore, tc.TotalOpsExecuted)

	for i := 1; i < 1000; i++ {
		cat := ClassifyExecution(tc)
		score := ComputeComplexityScore(tc)
		adj := ComputeGasAdjustment(cat, score, tc.TotalOpsExecuted)
...
func TestComputeGasAdjustment_Pure_FullDiscount(t *testing.T) {
	adj := ComputeGasAdjustment(ExecCategoryPure, 0, 200)
	if adj != -25 {
...
func TestComputeGasAdjustment_Pure_TooFewOps(t *testing.T) {
	adj := ComputeGasAdjustment(ExecCategoryPure, 0, 3)
	if adj != 0 {
...
func TestComputeGasAdjustment_Pure_LinearRamp(t *testing.T) {
	...
	adj100 := ComputeGasAdjustment(ExecCategoryPure, 0, 100)
	if adj100 != -25 {
...
func TestComputeGasAdjustment_Mixed_NoAdjustment(t *testing.T) {
	adj := ComputeGasAdjustment(ExecCategoryMixed, 20, 200)
	if adj != 0 {
...
func TestComputeGasAdjustment_Complex_MaxPenalty(t *testing.T) {
	adj := ComputeGasAdjustment(ExecCategoryComplex, 100, 500)
	if adj != 10 {
		t.Errorf("Complex with score 100 should get +10, got %d", adj)
	}
}

func TestComputeGasAdjustment_Complex_LinearPenalty(t *testing.T) {
	// score at complexPenaltyThreshold (25): 0 penalty
	adj25 := ComputeGasAdjustment(ExecCategoryComplex, 25, 500)
	if adj25 != 0 {
		t.Errorf("Complex at threshold should get 0, got %d", adj25)
	}

	// score between threshold and max
	adj50 := ComputeGasAdjustment(ExecCategoryComplex, 50, 500)
	t.Logf("Complex at score 50: adj=%d", adj50)
	if adj50 <= 0 || adj50 > 10 {
		t.Errorf("Complex at score 50 should get 1-10, got %d", adj50)
	}
}
...
func TestComputeGasAdjustment_IntegerDivision_NeverPanic(t *testing.T) {
	...
	for _, cat := range categories {
		for _, score := range scores {
			for _, op := range ops {
				// Must not panic
				_ = ComputeGasAdjustment(cat, score, op)
			}
		}
	}
}
