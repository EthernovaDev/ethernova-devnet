...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *genesisT.Genesis, overrides *ChainOverrides) (ctypes.ChainConfigurator, common.Hash, error) {
	...
	if (stored == common.Hash{}) {
		...
		if _, err := ethernovaPatchConfigIfNeeded(genesis.Config, 0); err != nil {
	...
	if storedcfg == nil {
		...
		if _, err := ethernovaPatchConfigIfNeeded(newcfg, head.Number.Uint64()); err != nil {
	...
	if updated, err := ethernovaPatchConfigIfNeeded(storedcfg, head.Number.Uint64()); err != nil {
		return newcfg, stored, err
	} else if updated {
